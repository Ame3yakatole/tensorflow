...
absl::flat_hash_set<int> GetDynamicInputTags(
    const OptimizationParameters& params) {
  absl::flat_hash_set<int> tags;
  if (params.learning_rate().has_dynamic()) {
    tags.insert(params.learning_rate().dynamic().tag());
  }
  tags.merge(GetDynamicHyperParameterTags(params));
  return tags;
}

absl::flat_hash_set<int> GetDynamicHyperParameterTags(
    const OptimizationParameters& params) {
  absl::flat_hash_set<int> tags;
  switch (params.parameters_case()) {
    case OptimizationAlgorithm::kFrequencyAwareAdagrad:
...
bool UsesDynamicInputs(const OptimizationParameters& params) {
  return !GetDynamicInputTags(params).empty();
}
